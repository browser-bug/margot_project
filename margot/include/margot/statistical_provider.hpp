...
class StatisticalProvider : public CircularBuffer<T> {
  ...
  static_assert(std::is_arithmetic<T>::value,
                "Error: in the statistical provider context, the template T must be an arithmetic type");
 ...
 private:
  ...
  inline statistical_type compute_average(void) {
    ...
    if (average_computed_time < CircularBuffer<T>::last_change) {
      previous_average = margot::average<typename CircularBuffer<T>::container_type, statistical_type>(
  ...
  inline statistical_type compute_standard_deviation(void) {
    ...
    if (stddev_computed_time < CircularBuffer<T>::last_change) {
      previous_stddev = margot::stddev<typename CircularBuffer<T>::container_type, statistical_type>(
          CircularBuffer<T>::buffer, compute_average());
...
struct monitor_utils<T, DataFunctions::AVERAGE, statistical_t> {
  /**
   * @brief The type of the average value, which is equal to StatisticalProvider<T,
   * statistical_t>::statistical_type
   */
...
struct monitor_utils<T, DataFunctions::STANDARD_DEVATION, statistical_t> {
  /**
   * @brief The type of the standard devation value, which is equal to StatisticalProvider<T,
   * statistical_t>::statistical_type
   */
  using value_type = typename StatisticalProvider<T, statistical_t>::statistical_type;
