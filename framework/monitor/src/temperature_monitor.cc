...
namespace margot
{
	...
	temperature_monitor_t::temperature_monitor_t(const std::size_t window_size, const std::size_t min_size):
		monitor_t( window_size, min_size )
	{
	}


	temperature_monitor_t::value_type temperature_monitor_t::temperature_sensor_t::measure(void)
	{
		...
		for (auto& sensor : sensors)
		{
			...
			if (pedantic_check())
				if ( result < 0 )
				{
					throw std::runtime_error("[temperature_monitor_t] Error: Unable to retrieve monitor informations");
				}
		...
		if (ns > 0)
		{
			return static_cast<temperature_monitor_t::value_type>(avg / ns);
		}

		return static_cast<temperature_monitor_t::value_type>(0);
	}



	temperature_monitor_t::temperature_sensor_t::~temperature_sensor_t(void)
	{
		sensors_cleanup();
		sensors.clear();
	}
	...
	temperature_monitor_t::temperature_sensor_t::temperature_sensor_t( void ): ns(n_core_sensors()), nc(sysconf(_SC_NPROCESSORS_ONLN))
	{
		// initialize the sensors
		if (!initialized)
		{
