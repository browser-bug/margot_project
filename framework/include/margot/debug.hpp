...
template <class OperatingPointSegment, std::size_t number_of_elements,
          std::size_t current_index = number_of_elements - 1>
struct segment_to_string {
  static void fill_configuration(const OperatingPointSegment& segment,
    ...
    description[current_index] =
        std_dev != 0 ? mean_value_str + " +- " + std::to_string(std_dev) : mean_value_str;

    // recursively compute the other fields
    segment_to_string<OperatingPointSegment, number_of_elements, current_index - 1>::fill_configuration(
        segment, description);
  }
};
...
inline void print_whole_op(const typename Knowledge<OperatingPoint>::OperatingPointPtr& op,
  ...
  segment_to_string<software_knob_segment_t, OperatingPoint::number_of_software_knobs>::fill_configuration(
      software_knobs, knob_values);
  ...
  segment_to_string<metric_segment_t, OperatingPoint::number_of_metrics>::fill_configuration(metrics,
                                                                                             metric_values);
...
inline void print_conf_with_value(const typename Knowledge<OperatingPoint>::OperatingPointPtr& op,
                                  const value_type value, const std::string& prefix,
