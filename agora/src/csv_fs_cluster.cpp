...
CsvClusterStorage::CsvClusterStorage(const FsConfiguration &configuration)
        : FsCluster(configuration), csv_separator(configuration.csv_separator) {
    cluster_dir = configuration.csv_storage_root_path / "clusters";
...
void CsvClusterStorage::store_cluster(const application_id &app_id, const margot::heel::block_model &description,
                                      const cluster_model &cluster) {
    ...
    for (const auto &centroid : cluster.centroids) {
        out << centroid.first;
        for (const auto &value : centroid.second) out << ',' << value;
        out << "\n";
    }
}
