...
class View {
 ...
 public:
   ...
   */
  View(void) {
    evaluate = [](const OperatingPointPtr& target_op) { return static_cast<value_type>(0); };
  }
   ...
   * is already in view.
   */
  inline void add(const OperatingPointPtr& new_op) { sorted_knowledge.emplace(evaluate(new_op), new_op); }
   ...
   */
  inline void clear(void) { sorted_knowledge.clear(); }
   ...
   */
  inline std::size_t size(void) const { return sorted_knowledge.size(); }
   ...
   */
  inline bool empty(void) const { return sorted_knowledge.empty(); }
   ...
   */
  inline value_type evaluate_op(const OperatingPointPtr target_op) const { return evaluate(target_op); }
