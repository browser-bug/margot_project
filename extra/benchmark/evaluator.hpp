...
class Launcher {
 ...
 public:
  ...
  ~Launcher(void) { plot<TimeType>(experiment_name, results); }

  Launcher(const Launcher&) = delete;
  Launcher(Launcher&&) = delete;

  inline void set_number_of_ops(std::vector<int> ops) { number_of_ops = std::move(ops); }
    ...
    for (const int num_ops : number_of_ops) {
      for (int i = 0; i < total_number_of_runs; ++i) {
        ...
        const uint64_t elapsed_time =
            std::chrono::duration_cast<std::chrono::nanoseconds>(stopping_point - starting_point).count();
