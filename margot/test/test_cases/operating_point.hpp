...
class OperatingPoint : public CxxTest::TestSuite {
 public:
  ...
  void test_getter_methods(void) {
    ...
    // check software knobs
    const auto k1_avg = margot::op_utils<OperatingPointType, margot::OperatingPointSegments::SOFTWARE_KNOBS,
                                         margot::BoundType::LOWER, 0, 0>::get(op);
    TS_ASSERT_EQUALS(k1_avg, 1);
    const auto k1_lb1 = margot::op_utils<OperatingPointType, margot::OperatingPointSegments::SOFTWARE_KNOBS,
                                         margot::BoundType::LOWER, 0, 1>::get(op);
    TS_ASSERT_EQUALS(k1_lb1, 1);
    const auto k1_lb2 = margot::op_utils<OperatingPointType, margot::OperatingPointSegments::SOFTWARE_KNOBS,
                                         margot::BoundType::LOWER, 0, 2>::get(op);
    TS_ASSERT_EQUALS(k1_lb2, 1);
    const auto k2_avg = margot::op_utils<OperatingPointType, margot::OperatingPointSegments::SOFTWARE_KNOBS,
                                         margot::BoundType::UPPER, 1, 0>::get(op);
    TS_ASSERT_EQUALS(k2_avg, 2);
    const auto k2_lb1 = margot::op_utils<OperatingPointType, margot::OperatingPointSegments::SOFTWARE_KNOBS,
                                         margot::BoundType::UPPER, 1, 1>::get(op);
    TS_ASSERT_EQUALS(k2_lb1, 2);
    const auto k2_lb2 = margot::op_utils<OperatingPointType, margot::OperatingPointSegments::SOFTWARE_KNOBS,
                                         margot::BoundType::UPPER, 1, 2>::get(op);
    TS_ASSERT_EQUALS(k2_lb2, 2);

    // check metrics
    const auto m1_avg = margot::op_utils<OperatingPointType, margot::OperatingPointSegments::METRICS,
                                         margot::BoundType::LOWER, 0, 0>::get(op);
    TS_ASSERT_DELTA(m1_avg, 3, float_epsilon);
    const auto m1_lb1 = margot::op_utils<OperatingPointType, margot::OperatingPointSegments::METRICS,
                                         margot::BoundType::LOWER, 0, 1>::get(op);
    TS_ASSERT_DELTA(m1_lb1, 2.9, float_epsilon);
    const auto m1_lb2 = margot::op_utils<OperatingPointType, margot::OperatingPointSegments::METRICS,
                                         margot::BoundType::LOWER, 0, 2>::get(op);
    TS_ASSERT_DELTA(m1_lb2, 2.8, float_epsilon);
    const auto m2_avg = margot::op_utils<OperatingPointType, margot::OperatingPointSegments::METRICS,
                                         margot::BoundType::UPPER, 1, 0>::get(op);
    TS_ASSERT_DELTA(m2_avg, 4, float_epsilon);
    const auto m2_lb1 = margot::op_utils<OperatingPointType, margot::OperatingPointSegments::METRICS,
                                         margot::BoundType::UPPER, 1, 1>::get(op);
    TS_ASSERT_DELTA(m2_lb1, 4.1, float_epsilon);
    const auto m2_lb2 = margot::op_utils<OperatingPointType, margot::OperatingPointSegments::METRICS,
                                         margot::BoundType::UPPER, 1, 2>::get(op);
  ...
  void test_enumerator_methods(void) {
    ...
    static_assert(margot::op_field_enumerator<OperatingPointType,
                                              margot::OperatingPointSegments::SOFTWARE_KNOBS, 0>::get() == 0,
                  "Error: unable to assign a correct id to target the Operating Point field");
    static_assert(margot::op_field_enumerator<OperatingPointType,
                                              margot::OperatingPointSegments::SOFTWARE_KNOBS, 1>::get() == 1,
                  "Error: unable to assign a correct id to target the Operating Point field");
    static_assert(
        margot::op_field_enumerator<OperatingPointType, margot::OperatingPointSegments::METRICS, 0>::get() ==
            2,
        "Error: unable to assign a correct id to target the Operating Point field");
    static_assert(
        margot::op_field_enumerator<OperatingPointType, margot::OperatingPointSegments::METRICS, 1>::get() ==
            3,
        "Error: unable to assign a correct id to target the Operating Point field");
  }
};
