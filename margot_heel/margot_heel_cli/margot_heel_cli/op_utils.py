...
def get_field_value( op_model, field ):
	"""
	...
	try:
		return float(op_model.metrics[field])
	except KeyError:
...
def dominate( op1, op2, directions ):
	"""
	This function returns True iff op1 dominate op2 accordinf to directions
	"""
	...
	for field_name in directions:
		...
		# get the values
		value_op_1 = get_field_value(op1, field_name)
		value_op_2 = get_field_value(op2, field_name)
...
def print_op( op ):
	...
	print('\tSoftware-knobs:')
	for param_name in op.knobs:
		print('\t\t{0} -> {1}'.format(param_name, op.knobs[param_name]))

	print('\tMetrics:')
	for metric_name in op.metrics:
		print('\t\t{0} -> {1}'.format(metric_name, op.metrics[metric_name]))
