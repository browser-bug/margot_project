create_doe <- function(knobs_config_list, doe_options, map_to_input = TRUE, algorithm = "dmax")
{
  suppressMessages(suppressPackageStartupMessages(library(DiceDesign)))
  
  if (algorithm == "full_factorial")
  {
    design <- expand.grid(knobs_config_list)
  } else
  {
    ...
    if (map_to_input)
    {
      # If map to input max the number of inputs to the number of combinations of the knobs configurations
    ...
    design <- switch(algorithm, strauss = straussDesign(nobs, ndim, eps), dmax = dmaxDesign(nobs, ndim, eps), lhs = lhsDesign(nobs, ndim), wsp = {
