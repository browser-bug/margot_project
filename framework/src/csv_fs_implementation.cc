...
struct csv_parser_t {
  ...
  inline std::string get(void) const { return current_row_raw; }
...
CsvStorage::CsvStorage(const std::string& storage_root_path)
    : storage_main_folder(storage_root_path),
...
void CsvStorage::store_description(const application_description_t& description) {
  ...
  if (out.is_open()) {
    ...
    throw std::runtime_error("Csv manager: unable to open/create file \"" +
                             get_knobs_name(description.application_name) + "\"");
  }
  ...
  if (out.is_open()) {
    ...
    warning("Csv manager: unable to open/create file \"", get_metrics_name(description.application_name),
            "\"");
    throw std::runtime_error("Csv manager: unable to open/create file \"" +
                             get_metrics_name(description.application_name) + "\"");
  }
  ...
  if (out.is_open()) {
    ...
    warning("Csv manager: unable to open/create file \"", get_features_name(description.application_name),
            "\"");
    throw std::runtime_error("Csv manager: unable to open/create file \"" +
                             get_features_name(description.application_name) + "\"");
  }
  ...
  if (out.is_open()) {
    ...
    out << "number_configurations_per_iteration," << description.number_configurations_per_iteration
        << std::endl;
    out << "number_observations_per_configuration," << description.number_observations_per_configuration
        << std::endl;
    ...
    warning("Csv manager: unable to open/create file \"", get_doe_info_name(description.application_name),
            "\"");
    throw std::runtime_error("Csv manager: unable to open/create file \"" +
                             get_doe_info_name(description.application_name) + "\"");
  }
                  ...
                  if (property_name.compare("k_value") == 0) {
                    ...
                    if (property_name.compare("minimum_distance") == 0) {
                      ...
                      if (property_name.compare("doe_limits") == 0) {
                        ...
                        warning("Csv manager: unknown doe property \"" + property_name + "\" with value \"" +
                                property_value + "\"");
                      }
                    }
                  }
...
void CsvStorage::store_model(const application_description_t& description, const model_t& model) {
  ...
  out << description.metrics[number_of_metrics].name << "_avg," << description.metrics[number_of_metrics].name
      << "_std" << std::endl;
  ...
  const int number_of_fields =
      number_of_metric_fields + description.knobs.size() + description.features.size();
