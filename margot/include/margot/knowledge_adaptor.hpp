...
class KnowledgeAdaptor {
 ...
 public:
   ...
   */
  using Container = std::array<FieldAdaptorPtr,
                               OperatingPoint::number_of_software_knobs + OperatingPoint::number_of_metrics>;

  /**
   * @brief Default constructor, initialize all the adaptors to a null pointer
   */
  KnowledgeAdaptor(void) { adaptors.fill(nullptr); }
  ...
  inline void emplace(const Monitor<T, statistical_t>& monitor) {
    adaptors[op_field_enumerator<OperatingPoint, target_segment, target_field_index>::get()].reset(
        new OneSigmaAdaptor<OperatingPoint, target_segment, target_field_index, inertia, coefficient_type>(
            monitor));
  }
   ...
   */
  inline void clear(void) { adaptors.fill(nullptr); }
