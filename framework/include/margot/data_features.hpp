...
struct dont_care {
   ...
   */
  constexpr bool operator()(const T&, const T&) const { return true; }
};
...
struct data_features_admissible {
  static_assert(sizeof...(cfs) + 1 <= DataFeature{}.size(),
                "Error: mismatch number of comparison function to validate a data feature");
  ...
  inline bool operator()(const DataFeature& f1, const DataFeature& f2) const {
    return comparison_function()(f1[f1.size() - (sizeof...(cfs) + 1)],
                                 f2[f1.size() - (sizeof...(cfs) + 1)]) &&
...
struct normalize_datafeatures {
  ...
  inline void operator()(DataFeature& origin, DataFeature& f1, DataFeature& f2) const {
    ...
    const auto coord_min =
        std::min(std::min(std::get<index>(origin), std::get<index>(f1)), std::get<index>(f2));
    const auto coord_max =
        std::max(std::max(std::get<index>(origin), std::get<index>(f1)), std::get<index>(f2));
 ...
 */
template <class DataFeature, class container_iterator, FeatureDistanceType distance_type,
          FeatureComparison... cfs>
...
struct data_feature_selector<DataFeature, container_iterator, FeatureDistanceType::EUCLIDEAN, cfs...> {
   ...
   */
  container_iterator operator()(const DataFeature& origin, const container_iterator& best,
                                const container_iterator& target) const {
    // instantiate the object to compare the data features
    const data_features_admissible<DataFeature, cfs...> is_valid =
        data_features_admissible<DataFeature, cfs...>();
