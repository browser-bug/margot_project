...
class Rank : public CxxTest::TestSuite {
 ...
 public:
  void setUp(void) {
    ...
    op_list.emplace_back(new MyOperatingPoint(
        {{2, 3}, {margot::Distribution<float>(5, 0.1), margot::Distribution<float>(6, 0.1)}}));
    op_list.emplace_back(new MyOperatingPoint(
        {{3, 4}, {margot::Distribution<float>(7, 0.1), margot::Distribution<float>(8, 0.1)}}));
  }
  ...
  void test_add(void) {
    ...
    valid_ops.reset(new margot::Rank<MyOperatingPoint, margot::RankObjective::MAXIMIZE,
                                     margot::FieldComposer::SIMPLE, first_metric_1sigma>(1.0f));
