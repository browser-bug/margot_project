...
#ifndef MARGOT_MONITOR_PAPI_MONITOR_HEADER
#define MARGOT_MONITOR_PAPI_MONITOR_HEADER
...
namespace margot
{
	 ...
	 *
	 */
	class papi_monitor_t : public monitor_t<long long int>
	{
		public:
			 ...
			 * exception is thrown.
			 */
			papi_monitor_t( const PapiEvent& event, const std::size_t observation_size = 1, const std::size_t min_size = 1);
			 ...
			 */
			papi_monitor_t( const papi_monitor_t& source );
			 ...
			 */
			papi_monitor_t( papi_monitor_t&& source );
			 ...
			 * @param [in] source The source asrtm
			 */
			papi_monitor_t& operator=(const papi_monitor_t& source ) = default;
			 ...
			 * @param [in] source The source asrtm
			 */
			papi_monitor_t& operator=( papi_monitor_t&& source ) = default;
			...
			~papi_monitor_t( void )
			{
				papi_interface_if::get_instance().erase(event);
			}
			...
			*/
			...
			{
				papi_interface_if::get_instance().start();
			}
		...
		private:
			...
			class papi_interface_if
			{
					 ...
					 */
					using event_values_t = std::vector<papi_monitor_t::value_type>;
				...
				public:
					 ...
					 */
					inline event_ptr_t get_nullpointer( void )
					{
					 ...
					 */
					inline papi_monitor_t::value_type get_value( const event_ptr_t& event )
					{
						if (pedantic_check())
						{
							if (event != observed_events.end())
							{
								return static_cast<papi_monitor_t::value_type>(values[event->second.position]);
							}
							else
							{
								return static_cast<papi_monitor_t::value_type>(0);
							}
						}
						else
						{
...
#endif // MARGOT_MONITOR_PAPI_MONITOR_HEADER
