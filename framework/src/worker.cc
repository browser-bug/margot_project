...
#include <unistd.h>
inline int get_tid(void) { return syscall(SYS_gettid); }
...
void handle_incoming_message(const message_t& new_message) {
  ...
  if (message_type.compare("/info") == 0) {
    ...
    // log the event
    pedantic("Thread ", get_tid(), ": received information about application \"", application_name, "\"",
             application_name);
  ...
  if (message_type.compare("/observation") == 0) {
    ...
    // log the event
    pedantic("Thread ", get_tid(), ": received a new observation for \"", application_name, "\": \"",
             observation, "\"");
