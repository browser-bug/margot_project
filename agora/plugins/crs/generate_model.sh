...
# STEP 1: dump the required information from the database
python $METRIC_ROOT/data_dumper.py --storage_type "$STORAGE_TYPE" --storage_address "$STORAGE_ADDRESS" --storage_username "$STORAGE_USERNAME" --storage_password "$STORAGE_PASSWORD" --observation_name "$OBSERVATION_CONTAINER_NAME" --features_name "$FEATURES_CONTAINER_NAME" --model_name "$MODEL_CONTAINER_NAME" --knobs_name "$KNOBS_CONTAINER_NAME" --target_metric "$METRIC_NAME" > $METRIC_ROOT/stdout.log 2> $METRIC_ROOT/stderr.log

# STEP 2: generate the actual model
Rscript $METRIC_ROOT/model.r $METRIC_NAME $METRIC_ROOT >> $METRIC_ROOT/stdout.log 2>> $METRIC_ROOT/stderr.log

# STEP 3: update the database with the new information
python $METRIC_ROOT/data_uploader.py --storage_type "$STORAGE_TYPE" --storage_address "$STORAGE_ADDRESS" --storage_username "$STORAGE_USERNAME" --storage_password "$STORAGE_PASSWORD" --observation_name "$OBSERVATION_CONTAINER_NAME" --features_name "$FEATURES_CONTAINER_NAME" --model_name "$MODEL_CONTAINER_NAME" --knobs_name "$KNOBS_CONTAINER_NAME" --target_metric "$METRIC_NAME" >> $METRIC_ROOT/stdout.log 2>> $METRIC_ROOT/stderr.log
