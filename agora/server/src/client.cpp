...
po::options_description get_options(application_options &app_opts) {
    ...
    communication_opts.add_options()
      ...
      ("sleep", po::value<long int>(&app_opts.sleep_time_ms)->default_value(1000), "The sleep time (in MICROSECONDS) that the client has to wait before sending a new message.")
...
void task(long int sleep_time_ms, const shared_ptr<agora::RemoteHandler> remote) {
    ...
    while (true) {
        ...
        if (!remote->recv_message(new_incoming_message)) break;
        std::cout << "Received new message from server: topic [" + new_incoming_message.topic + "] payload [" +
                             new_incoming_message.payload + "]"
                  << std::endl;
