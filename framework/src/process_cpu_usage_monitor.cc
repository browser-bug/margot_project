/* core/process_cpu_usage_monitor.hpp
...
// make sure to enable hardware counter only in linux environment
#ifdef __linux__
#define WITH_HARDWARE_COUNTER
#else
#undef WITH_HARDWARE_COUNTER
#endif  // __linux__
...
void getProcessTimeHard(timespec& process_time) { clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &process_time); }
#endif
...
ProcessCpuMonitor::ProcessCpuMonitor(CounterType __attribute__((unused)) counter_type,
...
ProcessCpuMonitor::ProcessCpuMonitor(const size_t window_size) : Monitor(window_size) {
  getProcessTime = getProcessTimeSoft;
  started = false;
}

void ProcessCpuMonitor::start() {
  if (started) {
    return;
  }
...
void ProcessCpuMonitor::stop() {
  ...
  uint64_t elapsedTimeProcess = static_cast<uint64_t>(
      (process_difference.tv_sec * static_cast<uint64_t>(1000000000)) + process_difference.tv_nsec);

  // compute the percentage
  value_type percentage =
      static_cast<value_type>(elapsedTimeProcess) / static_cast<value_type>(elapsedTimeSystem);
