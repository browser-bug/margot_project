...
pid_t Launcher::start_plugin(const fs::path &exec_script_path, const fs::path &config_file_path) const {
    ...
    if (plugin_pid == 0) {
        ...
        throw std::runtime_error("Launcher: unable to exec the script \"" + exec_script_path.string() +
                                 "\", errno=" + std::to_string(errno));
...
void Launcher::initialize_workspace(const application_id &app_id) {
    ...
    if (fs::exists(plugin_destination_path)) logger->warning("Launcher: the plugin working directory already exists.");
...
void Launcher::clear_workspace() {
    ...
    if (ec.value() != 0) logger->warning("Launcher: unable to remove \"", plugin_working_dir.string(), "\", err: ", ec.message());
}
...
pid_t Launcher::launch(const PluginConfiguration &env_configuration) {
    ...
    if (!fs::exists(script_path)) {
        ...
        throw std::runtime_error("Launcher: the plugin script file [" + script_path.string() +
                                 "] cannot be found. Have you initiliazed the plugin launcher?");
    }
...
pid_t Launcher::launch() {
    ...
    if (!fs::exists(config_path)) {
        ...
        throw std::runtime_error(
                "Launcher: the environmental configuration file cannot be found. Is this the first time you launch this plugin?");
    }
