...
namespace margot
{
  ...
  PapiMonitor::PapiMonitor( void ): Monitor()
  {
    event = papi_interface_if::get_instance().get_nullpointer();
  }


  PapiMonitor::PapiMonitor( const PapiEvent& event, const std::size_t observation_size ): Monitor(observation_size)
  {
  ...
  PapiMonitor::papi_interface_if::event_ptr_t PapiMonitor::papi_interface_if::insert(const margot::PapiEvent& event)
  {
    // acquire the lock
