...
class ConstraintHandler {
 ...
 public:
   ...
   */
  virtual void set_field_adaptor(
      const KnowledgeAdaptor<OperatingPoint, error_coef_type>& runtime_information) = 0;
...
struct constraint<ComparisonFunctions::GREATER> {
   ...
   */
  inline static constexpr BoundType get_bound(void) { return BoundType::LOWER; }
};
...
struct constraint<ComparisonFunctions::LESS> {
   ...
   */
  inline static constexpr BoundType get_bound(void) { return BoundType::UPPER; }
};
 ...
 */
template <class OperatingPoint, OperatingPointSegments segment, std::size_t field_index, int sigma,
          class ConstraintGoal, typename error_coef_type = float>
class Constraint : public ConstraintHandler<OperatingPoint, error_coef_type> {
 ...
 public:
   ...
   */
  using SiblingConstraintPtr =
      typename ConstraintHandler<OperatingPoint, error_coef_type>::ConstraintHandlerPtr;
   ...
   */
  using ConstraintField =
      OPField<segment, helper::constraint<ConstraintGoal::comparison_function>::get_bound(), field_index,
              sigma, int>;
  ...
  SiblingConstraintPtr create_sibling(void) const {
    return SiblingConstraintPtr{
        new Constraint<OperatingPoint, segment, field_index, sigma, ConstraintGoal, error_coef_type>(
            target_goal)};
  }
   ...
   * @see ConstraintHandler
   */
  void add(const OperatingPointPtr& new_op) { knowledge_view.add(new_op); }
  ...
  OPStream narrow(const OPStream& ops) const {
    ...
    bool best_op_is_valid =
        target_goal.template check<view_type, value_type>(best_op_value, last_check_value);
   ...
   * @see ConstraintHandler
   */
  void clear(void) { blocked_ops.clear(); }
  ...
  void filter_initialize(OPStream& input, OPStream& blocked) {
    ...
    for (auto&& op : input) {
      if (!target_goal.template check<view_type, value_type>(knowledge_view.evaluate_op(op),
                                                             last_check_value)) {
  ...
  void update(OPStream& invalidated_ops, OPStream& validated_ops) {
    ...
    const error_coef_type coef =
        knowledge_adaptor ? knowledge_adaptor->get_error_coefficient() : static_cast<error_coef_type>(1.0f);
...
template <class OperatingPoint, OperatingPointSegments segment, std::size_t field_index, int sigma,
          class ConstraintGoal, typename error_coef_type>
void Constraint<OperatingPoint, segment, field_index, sigma, ConstraintGoal, error_coef_type>::dump(
    std::string prefix) {
  ...
  const auto coef =
      knowledge_adaptor ? knowledge_adaptor->get_error_coefficient() : static_cast<error_coef_type>(1);
  ...
  if (!blocked_ops.empty()) {
    ...
    for (const auto op_ptr : blocked_ops) {
      ...
      print_conf_with_value<OperatingPoint, typename MyView::value_type>(op_ptr, op_value, prefix,
                                                                         "Constraint Value");
      std::cout << prefix << std::endl;
    }
  } else {
