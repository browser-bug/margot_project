 ...
 */
template <class OperatingPoint, class state_id_type = std::string, typename priority_type = int,
...
class Asrtm {
  ...
  static_assert(
      traits::is_operating_point<OperatingPoint>::value,
 ...
 public:
  ...
  /**
   * @brief Forward declaration of the preferred op container
   */
  using operating_point_container_type = std::vector<operating_point_type>;
  ...
  Asrtm(Asrtm&& other) {
    ...
    runtime_information =
        std::move(KnowledgeAdaptor<OperatingPoint, error_coef_type>(other.runtime_information));
   ...
   *
   * @tparam T An object which implements the STL Container concept and stores Operating Points or
   * configurations
   *
  ...
  void create_new_state(const state_id_type& new_state_id) {
    ...
    if (state_it == application_optimizers.end()) {
      ...
      const auto return_value = application_optimizers.emplace(
          new_state_id, State<OperatingPoint, priority_type, error_coef_type>{});
  ...
  inline void change_active_state(const state_id_type& state_id) {
    ...
    assert(current_optimizer != application_optimizers.end() &&
           "Error: attempt to switch to a non-existent state");
  }
  ...
  template <OperatingPointSegments target_segment, std::size_t target_field_index, std::size_t inertia,
            class T, typename statistical_t>
  void add_runtime_knowledge(const Monitor<T, statistical_t>& monitor) {
    // lock the manger mutex, to ensure a consistent global state
    std::lock_guard<std::mutex> lock(manager_mutex);

    // set this information to the knowledge adaptor
    runtime_information.template emplace<target_segment, target_field_index, inertia, T, statistical_t>(
        monitor);
  ...
  configuration_type get_best_configuration(bool* configuration_changed = nullptr) {
    // lock the manger mutex, to ensure a consistent global state
    std::lock_guard<std::mutex> lock(manager_mutex);
    assert(proposed_best_configuration &&
           "Error: AS-RTM attempt to retrieve the best configuration from an empty state");
  ...
  void configuration_applied(void) {
    ...
    status = status != ApplicationStatus::DESIGN_SPACE_EXPLORATION
                 ? ApplicationStatus::TUNED
                 : ApplicationStatus::DESIGN_SPACE_EXPLORATION;
    ;
  }
  ...
  inline T get_mean(void) const {
    // lock the manger mutex, to ensure a consistent global state
    std::lock_guard<std::mutex> lock(manager_mutex);
    assert(application_configuration &&
           "Error: AS-RTM attempt to retrieve information from an empty application configutation");
    return application_configuration
               ? static_cast<T>(
                     Evaluator<
                         OperatingPoint, FieldComposer::SIMPLE,
  ...
  inline Asrtm<OperatingPoint, state_id_type, priority_type, error_coef_type> create_sibling(void) const {
    ...
    for (const auto& state : application_optimizers) {
      ...
      const auto result =
          cloned_manager.application_optimizers.emplace(state.first, state.second.create_sibling());
   ...
   * By convention, the measure should be a string that contains the equivalent
   * of an Operating Point, encoded in json as the configuration file.
   * The string is meant to be generated automatically from the high-level interface
   * to ensure consistency between the two end-points: the margot enhanced application,
   * and the agora remote application handler
   */
  inline void send_observation(const std::string &observation)
  {
    std::lock_guard<std::mutex> lock(manager_mutex);
   ...
   *
   * @details
   * The struct that parses the Operating Point from a string representation is meant to be automatically
   * generated by the high level interface, to ensure consistency between the two end-point of the
   * communication: agora and the margot enhanced application.
   */
  template <class OpConverter>
  void start_support_thread(const std::string& application_id, const std::string& broker_url,
    ...
    // get the application name ("app_name^block_name^version")
 ...
 private:
   ...
   */
  void set_single_point(const OperatingPoint& point) {
   ...
   */
  void set_model(const operating_point_container_type& model) {
    // lock the asrtm, since we are changing its data structure
    std::lock_guard<std::mutex> lock(manager_mutex);
  ...
  void local_application_handler(const std::string application_description) {
    ...
    // get my own id
    my_client_id = remote->get_my_client_id();

    // register to the application-specific topic (before send the welcome message)
    remote->subscribe(agora::MESSAGE_HEADER + "/" + app_id + "/" + my_client_id + "/#");

    // register to the application topic to receive the prediction
    remote->subscribe(agora::MESSAGE_HEADER + "/" + app_id + "/prediction");

    // register to the server welcome topic
    remote->subscribe(agora::MESSAGE_HEADER + "/welcome");
    ...
    while (true) {
      ...
      if (!remote->recv_message(new_incoming_message)) {
        ...
        logger->info("mARGOt support thread on retirement.");
        return;
      }

      const auto start_type_pos = new_incoming_message.topic.find_last_of('/');
      const auto message_type = new_incoming_message.topic.substr(start_type_pos);
  ...
  /**
   * @brief The identifier (pid) of this client
   */
...
void Asrtm<OperatingPoint, state_id_type, priority_type, error_coef_type>::dump(bool starting_point) const {
  ...
  std::cout << "# // Number of State(s) (optimization problems): " << application_optimizers.size()
            << std::endl;
