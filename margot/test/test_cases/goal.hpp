...
class Goal : public CxxTest::TestSuite {
 public:
  ...
  void test_check_data_functions(void) {
    ...
    monitor.push(1);
    TS_ASSERT_DELTA((mygoal.relative_error<float, float, margot::DataFunctions::AVERAGE>(monitor)), 0.5,
                    0.001);

    monitor.push(2.5);
    TS_ASSERT_DELTA((mygoal.absolute_error<float, float, margot::DataFunctions::AVERAGE>(monitor)), 0, 0.001);
    monitor.push(1);
    TS_ASSERT_DELTA((mygoal.absolute_error<float, float, margot::DataFunctions::AVERAGE>(monitor)), 1.0,
                    0.001);
  }
