...
void copy_folder(const std::string& input_folder, const std::string& output_folder) {
  ...
  while (std::getline(path_stream, current_path, '/')) {
    ...
    if (!is_created) {
      ...
      throw std::runtime_error("Launcher: unable to create the folder \"" + temporary_directory +
  ...
  if (cp_pid == 0) {
    ...
    warning("Launcher: unable to copy the folder \"", input_folder, "\" into \"", output_folder,
            "\", errno=", errno);
    throw std::runtime_error("Launcher: unable to copy the folder \"" + input_folder + "\" into \"" +
                             output_folder + "\", errno=" + std::to_string(errno));
  } else if (cp_pid < 0) {
  ...
  if (rc < 0) {
    ...
    throw std::runtime_error("Launcher: the \"cp\" process terminated with errno=" + std::to_string(errno) +
                             ", return code:" + std::to_string(return_code));
  }
}

void generate_environmental_file(const application_description_t& application,
                                 const std::string& destination_file_path, const std::string& metric_name,
                                 const std::string& plugin_root_path, const int iteration_counter) {
  ...
  config_file << "OBSERVATION_CONTAINER_NAME=\""
              << io::storage.get_observation_name(application.application_name) << "\"" << std::endl;
  config_file << "MODEL_CONTAINER_NAME=\"" << io::storage.get_model_name(application.application_name) << "\""
              << std::endl;
  config_file << "KNOBS_CONTAINER_NAME=\"" << io::storage.get_knobs_name(application.application_name) << "\""
              << std::endl;
  config_file << "FEATURES_CONTAINER_NAME=\"" << io::storage.get_features_name(application.application_name)
              << "\"" << std::endl;
  config_file << "DOE_CONTAINER_NAME=\"" << io::storage.get_doe_name(application.application_name) << "\""
              << std::endl;
  config_file << "DOE_INFO_CONTAINER_NAME=\"" << io::storage.get_doe_info_name(application.application_name)
              << "\"" << std::endl;
  ...
  config_file << "NUMBER_CONFIGURATIONS_PER_ITERATION=\"" << application.number_configurations_per_iteration
              << "\"" << std::endl;
  config_file << "NUMBER_OBSERVATIONS_PER_CONFIGURATION=\""
              << application.number_observations_per_configuration << "\"" << std::endl;
...
pid_t launch_plugin(const std::string& exec_script_path, const std::string& config_file_path) {
  ...
  if (plugin_pid == 0) {
    ...
    throw std::runtime_error("Launcher: unable to exec the script \"" + exec_script_path +
                             "\", errno=" + std::to_string(errno));
  } else if (plugin_pid < 0) {
    warning("Launcher: unable to fork for execuing the plugin errno=", errno);
    throw std::runtime_error("Launcher: unable to fork for execuing the plugin errno=" +
                             std::to_string(errno));
  }
...
void wait_plugin(const pid_t plugin_pid) {
  ...
  if (rc < 0) {
    ...
    throw std::runtime_error("Launcher: unable to wait the child \"" + std::to_string(plugin_pid) +
                             "\" errno=" + std::to_string(errno));
  }

  if (plugin_return_code != 0) {
    warning("Launcher: a plugin process terminated with return code:", plugin_return_code);
    throw std::runtime_error("Launcher: a plugin process terminated with return code:" +
                             std::to_string(plugin_return_code));
  }
}
...
template <>
void Launcher<LauncherType::ModelGenerator>::operator()(const application_description_t& application,
                                                        const uint_fast32_t iteration_counter) const {
  ...
  for (const auto& metric : application.metrics) {
    ...
    const std::string metric_root =
        workspace_root + "/" + application.application_name + "/model_" + metric.name;
    ...
    sh_util::generate_environmental_file(application, config_path, metric.name, metric_root,
                                         iteration_counter);
...
template <>
void Launcher<LauncherType::DoeGenerator>::operator()(const application_description_t& application,
                                                      const uint_fast32_t iteration_counter) const {
  // compute the path of the plugin that computes the metric
