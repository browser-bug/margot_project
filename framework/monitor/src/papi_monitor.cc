...
namespace margot
{
	...
	papi_monitor_t::papi_monitor_t( void ): monitor_t()
	{
		event = papi_interface_if::get_instance().get_nullpointer();
	}


	papi_monitor_t::papi_monitor_t( const PapiEvent& event, const std::size_t observation_size, const std::size_t min_size): monitor_t(observation_size, min_size)
	{
		this->event = papi_interface_if::get_instance().insert(event);
	}


	papi_monitor_t::papi_monitor_t( const papi_monitor_t& source )
	{
	...
	papi_monitor_t::papi_monitor_t( papi_monitor_t&& source )
	{
		// register the new reference
		this->event = papi_interface_if::get_instance().insert(static_cast<PapiEvent>(source.event->first));
	...
	papi_monitor_t::papi_interface_if::papi_interface_if( void )
	{
	...
	papi_monitor_t::papi_interface_if::event_ptr_t papi_monitor_t::papi_interface_if::insert(const margot::PapiEvent& event)
	{
		// acquire the lock
	...
	void papi_monitor_t::papi_interface_if::erase(const event_ptr_t& event)
	{
		// acquire the lock
		std::lock_guard<std::mutex> lock(mutex);
	...
	void papi_monitor_t::papi_interface_if::start( void )
	{
		// lock the operation
		std::lock_guard<std::mutex> lock(mutex);
	...
	void papi_monitor_t::papi_interface_if::stop( void )
	{
		// lock the operation
		std::lock_guard<std::mutex> lock(mutex);
