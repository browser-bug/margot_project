#ifndef MARGOT_TEST_UPDATE_HDR
#define MARGOT_TEST_UPDATE_HDR

#include "margot/asrtm.hpp"

#include "evaluator.hpp"

template <int number_of_changing_ops>
class UpdateFlat {
  ...
  margot::Goal<float, margot::ComparisonFunctions::GREATER_OR_EQUAL> goal;

 public:
  UpdateFlat(const int number_of_ops) {
    ...
    for (int i = 0; i < number_of_ops; ++i) {
      op_list.push_back({{i},
                         {margot::Distribution<float>(number_of_ops - i, 0.1),
    ...
    goal.set(static_cast<float>(number_of_changing_ops));
  }

  inline void operator()(void) { manager.find_best_configuration(); }
};

#endif  // MARGOT_TEST_UPDATE_HDR
