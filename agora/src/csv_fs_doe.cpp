...
CsvDoeStorage::CsvDoeStorage(const FsConfiguration &configuration) : FsDoe(configuration), csv_separator(configuration.csv_separator) {
...
void CsvDoeStorage::store_doe(const application_id &app_id, const margot::heel::block_model &description, const doe_model &doe) {
    ...
    for (const auto &doe_entry : doe.required_explorations) {
        ...
        for (const auto &knob : description.knobs) out << ',' << doe_entry.configuration.at(knob.name);
        out << "\n";
    }
}
...
void CsvDoeStorage::erase(const application_id &app_id) {
    ...
    if (ec.value() != 0) logger->warning("Csv doe: unable to remove \"", path.string(), "\", err: ", ec.message());
}
