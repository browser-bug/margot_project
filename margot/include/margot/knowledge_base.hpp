...
class Knowledge {
 ...
 public:
   ...
   */
  using OperatingPointList =
      std::unordered_map<configuration_type, OperatingPointPtr, hash<configuration_type> >;
  ...
  inline OperatingPointPtr add(const OperatingPointPtr& new_operating_point) {
    const auto result_pair = knowledge.emplace(new_operating_point->get_knobs(), new_operating_point);
   ...
   */
  inline void set(OperatingPointList& new_list) { knowledge.swap(new_list); }

  /**
   * @brief Removes all the Operating Points from the application knwoledge
   */
  inline void clear(void) { knowledge.clear(); }
   ...
   */
  inline typename OperatingPointList::const_iterator begin(void) const { return knowledge.begin(); }
   ...
   */
  inline typename OperatingPointList::const_iterator end(void) const { return knowledge.end(); }
   ...
   */
  inline bool empty(void) const { return knowledge.empty(); }
   ...
   */
  inline std::size_t size(void) const { return knowledge.size(); }
