...
std::vector<std::pair<std::string, unsigned long long int>> get_file_from_package(const std::string domain,
  ...
  while (found_file) {
    ...
    const std::string file_name(
        "/sys/class/powercap/intel-rapl/intel-rapl:" + std::to_string(package) +
    ...
    if (name_stream.good()) {
      ...
      if (domain_name.compare(0, domain.size(), domain) == 0) {
        const std::string file_name_to_be_read(
            "/sys/class/powercap/intel-rapl/intel-rapl:" + std::to_string(package) +
            (domain != "package"
                 ? "/intel-rapl:" + std::to_string(package) + ":" + std::to_string(domain_index)
                 : "") +
            "/energy_uj");
        std::ifstream ifs("/sys/class/powercap/intel-rapl/intel-rapl:" + std::to_string(package) +
                          (domain != "package"
                               ? "/intel-rapl:" + std::to_string(package) + ":" + std::to_string(domain_index)
                               : "") +
                          "/max_energy_range_uj");
...
std::vector<std::pair<std::string, unsigned long long int>> get_interested_file_list(
    const std::size_t max_number_packages, const std::string domain,
    const std::vector<std::size_t> target_packages = {}) {
...
std::vector<std::pair<unsigned long long int, unsigned long long int>> get_measure(
    const std::vector<std::pair<std::string, unsigned long long int>> files) {
...
// Default constructor
EnergyMonitor::EnergyMonitor(const std::size_t window_size)
    : EnergyMonitor(EnergyMonitor::Domain::Cores, {}, window_size) {}

// Domain specialized constructor
EnergyMonitor::EnergyMonitor(const Domain interested_domain, const std::vector<std::size_t> target_packages,
                             const std::size_t window_size)
    : Monitor<value_type>(window_size), previous_measure(0), started(false) {
...
void EnergyMonitor::stop(void) {
  if (started) {
    std::vector<std::pair<unsigned long long int, unsigned long long int>> actual_values = extractor();
    value_type accumulator = 0;

    for (std::size_t i = 0; i < previous_measure.size(); i++) {
      if (actual_values.at(i).first < previous_measure.at(i).first) {
        accumulator += static_cast<long double>(actual_values.at(i).second - previous_measure.at(i).first +
                                                actual_values.at(i).first) /
                       1000000;
      } else {
        accumulator +=
            static_cast<long double>(actual_values.at(i).first - previous_measure.at(i).first) / 1000000;
      }
    }
