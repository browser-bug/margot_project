create_doe <- function(knobs_config_list, doe_options, map_to_input = TRUE, algorithm = "strauss")
{
  ...
  if(map_to_input)
  {
    ...
    if( length( knobs_config_list ) > 1 )
    {
      ...
      ind <- row.names(design_space_grid)[which.min( rowSums( abs( t( apply(design_space_grid, 1, function(x){x - design[1, ]} )  ) ) ) )]
      for(i in 2:dim(design)[1])
      {
        # Find the index of the point closest to the i-th design point, discard already selected designs
        ind_temp <- which.min( rowSums( abs( t( apply( design_space_grid[-as.numeric(ind), ], 1, function(x){x - design[i, ]} ) ) ) ) )
