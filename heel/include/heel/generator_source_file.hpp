...
class source_file_generator {
 ...
 public:
  ...
  void write_header(const std::vector<std::filesystem::path>& path_configuration_files) {
    ...
    internal_write(file_handler, path_configuration_files);
    file_handler << "#endif // " << include_guard_def << std::endl << std::endl;
    close(file_handler);
  }

  inline void write_source(const std::vector<std::filesystem::path>& path_configuration_files) {
    std::ofstream file_handler;
    open(file_handler);
    internal_write(file_handler, path_configuration_files);
    close(file_handler);
  ...
  inline std::filesystem::path get_file_path(void) const { return file_path; }

 private:
  void open(std::ofstream& file_handler) {
  ...
  void close(std::ofstream& file_handler) {
    if (file_handler.is_open()) {
      file_handler.close();
    }
  }

  void internal_write(std::ofstream& file_handler,
                      const std::vector<std::filesystem::path>& path_configuration_files) {
    ...
    if (!path_configuration_files.empty()) {
      file_handler << "// Configuration files path: " << std::endl;
      std::for_each(
          path_configuration_files.begin(), path_configuration_files.end(),
          [&file_handler](const std::filesystem::path& p) { file_handler << "//\t- " << p << std::endl; });
    }
    ...
    std::for_each(required_headers.begin(), required_headers.end(),
                  [&file_handler](const std::string& header) {
