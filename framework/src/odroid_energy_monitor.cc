...
inline void readBigPower(long double* power) {
  ...
  if (fp == NULL) {
    throw std::runtime_error(
        "[OdroidEnergyMonitor] Error: unable to open the /sys/bus/i2c/drivers/INA231/2-0040/sensor_W file "
        "(BIG)");
  }
  ...
  if (result == EOF) {
    throw std::runtime_error(
        "[OdroidEnergyMonitor] Error: unable to parse the /sys/bus/i2c/drivers/INA231/2-0040/sensor_W file "
        "(BIG): reached end of file.");
  }
}
...
inline void readLittlePower(long double* power) {
  ...
  if (fp == NULL) {
    throw std::runtime_error(
        "[OdroidEnergyMonitor] Error: unable to open the /sys/bus/i2c/drivers/INA231/2-0045/sensor_W file "
        "(LITTLE)");
  }
  ...
  if (result == EOF) {
    throw std::runtime_error(
        "[OdroidEnergyMonitor] Error: unable to parse the /sys/bus/i2c/drivers/INA231/2-0045/sensor_W file "
        "(LITTLE): reached end of file.");
  }
}
...
inline void readMemoryPower(long double* power) {
  ...
  if (fp == NULL) {
    throw std::runtime_error(
        "[OdroidEnergyMonitor] Error: unable to open the /sys/bus/i2c/drivers/INA231/2-0041/sensor_W file "
        "(MEM)");
  }
  ...
  if (result == EOF) {
    throw std::runtime_error(
        "[OdroidEnergyMonitor] Error: unable to parse the /sys/bus/i2c/drivers/INA231/2-0041/sensor_W file "
        "(MEM): reached end of file.");
  }
}
...
inline void readGpuPower(long double* power) {
  ...
  if (fp == NULL) {
    throw std::runtime_error(
        "[OdroidEnergyMonitor] Error: unable to open the /sys/bus/i2c/drivers/INA231/2-0044/sensor_W file "
        "(GPU)");
  }
  ...
  if (result == EOF) {
    throw std::runtime_error(
        "[OdroidEnergyMonitor] Error: unable to parse the /sys/bus/i2c/drivers/INA231/2-0044/sensor_W file "
        "(GPU): reached end of file.");
  }
}
...
OdroidEnergyMonitor::OdroidEnergyMonitor(TimeUnit time_measure, const uint64_t polling_time_ms,
                                         const std::size_t window_size)
    : Monitor(window_size) {
  started = false;
  synchronous_thread_if_p =
      std::shared_ptr<synchronous_thread_if>(new synchronous_thread_if(polling_time_ms));
}
