 ...
 * @see Evaluator
 */
template <OperatingPointSegments segment, BoundType bound, std::size_t field, int sigma_std,
          typename T = float>
struct OPField {
  /**
...
class Evaluator<OperatingPoint, FieldComposer::SIMPLE, Field> {
 ...
 public:
   ...
   */
  using value_extractor =
      op_utils<OperatingPoint, Field::target_segment, Field::target_bound, Field::field_index, Field::sigma>;
 ...
 public:
  ...
  inline static value_type evaluate(const OperatingPointPtr& target_op, const Field field,
    ...
    return (value_extractor::get(target_op) * field.coefficient) +
           next_bit_extractor::evaluate(target_op, other...);
  }
};
