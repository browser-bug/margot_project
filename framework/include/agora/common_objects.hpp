...
struct sort_fields_operator {
  inline bool operator()(const knob_t& knob1, const knob_t& knob2) const { return (knob1.name < knob2.name); }
...
struct application_description_t {
  ...
  application_description_t(const std::string& application_name, application_knobs_t k,
                            application_features_t f, application_metrics_t m)
...
struct model_t {
  ...
  inline int column_size(void) const {
    return static_cast<int>(
        knowledge.empty() ? 0 : std::count(knowledge.front().begin(), knowledge.front().end(), ',') + 1);
  }
