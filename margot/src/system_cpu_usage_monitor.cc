...
inline void readTime(uint64_t* user, uint64_t* nice, uint64_t* system, uint64_t* idle) {
  ...
  int result = ::fscanf(fp.get(), "%*s %lu %lu %lu %lu %lu %lu %lu %lu %lu %lu", user, nice, system, idle,
                        &io_wait, &irq, &softirq, &steal, &guest, &guest_nice);
  ...
  if (result == EOF) {
    throw std::runtime_error(
        "[system_cpu_usage_t] Error: unable to parse the /proc/stat file: reached end of file.");
  }
}
...
SystemCpuMonitor::SystemCpuMonitor(const std::size_t window_size) : Monitor(window_size) { started = false; }

void SystemCpuMonitor::start() {
  if (started) {
    return;
  }
