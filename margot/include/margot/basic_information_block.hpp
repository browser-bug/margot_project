...
struct Data {
  static_assert(std::is_arithmetic<T>::value,
                "Error: in Data context, the template T must be an arithmetic type");
...
struct hash<Data<T> > {
   ...
   */
  std::size_t operator()(Data<T> const& datum) const { return std::hash<T>()(datum.mean); }
};
...
struct hash<Distribution<T> > {
   ...
   */
  std::size_t operator()(Distribution<T> const& datum) const { return std::hash<T>()(datum.mean); }
};
