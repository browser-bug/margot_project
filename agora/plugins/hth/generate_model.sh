...
Rscript $METRIC_ROOT/model.R $STORAGE_TYPE $STORAGE_ADDRESS $APPLICATION_NAME $METRIC_NAME $METRIC_ROOT $ITERATION_COUNTER $DOE_NAME $MINIMUM_DISTANCE $NUMBER_POINT_PER_DIMENSION $NUMBER_OBSERVATIONS_PER_POINT >> $METRIC_ROOT/stdout.log 2>> $METRIC_ROOT/stderr.log
