...
po::options_description get_options(application_options &app_opts) {
  ...
  storage_opts.add_options()
    ("storage-implementation", po::value<string>()->default_value("csv"), "The name of the storage used by agora [csv].")
    ("storage-address", po::value<string>(&app_opts.storage_address)->default_value(""), "A reference to the storage, depending on its actual implementation.")
    ("storage-username", po::value<string>(&app_opts.storage_username)->default_value(""), "The username for authentication purposes, if any.")
    ("storage-password", po::value<string>(&app_opts.storage_password)->default_value(""), "The password for authentication purposes, if any.");
  ...
  communication_opts.add_options()
    ...
    ("broker-url", po::value<string>(&app_opts.broker_url)->default_value("127.0.0.1:1883"), "The url of the MQTT broker.")
    ("broker-username", po::value<string>(&app_opts.broker_username)->default_value(""), "The username for authentication purposes, if any.")
    ("broker-password", po::value<string>(&app_opts.broker_password)->default_value(""), "The password for authentication purposes, if any.")
    ("broker-ca", po::value<string>(&app_opts.broker_certificate)->default_value(""), "The path to the broker certificate (e.g. broker.crt), if any.")
    ("client-ca", po::value<string>(&app_opts.client_certificate)->default_value(""), "The path to the client certificate (e.g. client.crt), if any.")
    ("client-private-key", po::value<string>(&app_opts.client_key)->default_value(""), "The path to the private key (e.g. client.key), if any.")
  ...
  internal_opts.add_options()
    ("min-log-level", po::value<string>()->default_value("info"), "The minimum level of logging\n[disabled, warning, info, pedantic, debug].")
...
inline auto resolve_log_level = [](std::string log_level) -> agora::LogLevel {
    ...
    if (itr != log_implementations.end()) return itr->second;
...
inline auto resolve_storage_implementation = [](std::string storage_implementation) -> agora::StorageType {
    ...
    if (itr != storage_implementations.end()) return itr->second;
...
inline auto resolve_mqtt_implementation = [](std::string mqtt_implementation) -> agora::RemoteType {
    ...
    if (itr != mqtt_implementations.end()) return itr->second;
